...
class process_http_request:
    ...
    def callback_http_request(self, http_request, param, payload, payload_num,waf_flag):
        ...
        :return:
        '''
        if payload_num == 3 and waf_flag !=1:
            http_request['headers']['X-Forwarded-For'] = payload
            http_request['headers']['User-Agent'] = payload
    ...
    def payload_scan(self,waf_flag):
        ...
        for item in self.callback_param_list(self.test_json):
            if item[1] != 'Json':
                ...
                for i in range(0, 4):  # 追加/替换/数组/header扫描/bypasswaf/场景合并
                    for payload in self.payload_list:
                        ...
                        if 'GET' in data['method']:
                            ...
                            con_time = requests.post(url=data['url'], headers=data['headers'], data=data['body'],
                                                     verify=False,
                                                     proxies=proxies).elapsed.total_seconds()
                            ...
                            if flag:
                                print(
                                    'exist SQL Vulnerability___(Regular matching)   ' + data['url'] + '           ' +
                                    data[
                                        'body'])
                                self.output_data(
                                    'exist SQL Vulnerability___(Regular matching)   ' + data['url'] + '\n\n'+str(data['headers'])+'\n'+data['body'])
                            ...
                            if flag_time:
                                print(
                                    'exist SQL Vulnerability___(Delay/Time Response)   ' + str(data['url']) + '           ' +str(data['body']))
        ...
        if bool(t):
            for vul_flag in t:
                ...
                if 'GET' in data['method']:
                    ...
                    print(
                        'exist SQL Vulnerability___(Text comparison)  ' + datalist[vul_flag] + '           ' + data_body[vul_flag])
                    self.output_data(
                        'exist SQL Vulnerability___(Text comparison)  ' + datalist[vul_flag] + '           ' + '\n'+str(data['headers']) + '\n'+data_body[vul_flag])
                    # self.output_data(
                    #     'exist SQL Vulnerability___(Text comparison)  '+ data_body[vul_flag]+htm_list[vul_flag]+'\n\n\n\n\n'+htm_list[vul_flag+1])
        ...
        if waf_flag == 1:
            ...
            for ms in range(1,4):
                if ms == 1:
                    ...
                    continue
                # elif ms ==3:
                #     if 'json' in self.test_json['Accept']:
                #         continue
                #     self.test_json['body'] = self.test_json['body']+'&uname=admin'
                #     self.payload_scan(waf_flag)
                #     self.test_json = self.get_json()
                #     continue
