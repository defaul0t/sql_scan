...
class process_http_request:
    ...
    def process_payload(self, json_data, num, payload):
        ...
        if num == 0:
            ...
            # print(json_data)
        elif num == 1 and 'json' in self.test_json['headers']['Content-Type']==False:  # 改为数组
            json_data = str(json_data) + payload
        elif num == 2:
            self.test_json = self.test_json
    ...
    def callback_http_request(self, http_request, param, payload, payload_num,waf_flag):
        ...
        :return:
        '''
        # if payload_num == 2 and waf_flag != 1:
    ...
    def payload_scan(self,waf_flag):
        ...
        for item in self.callback_param_list(self.test_json):
            if item[1] != 'Json':
                ...
                for i in range(0, 3):  # 追加/替换/数组/header扫描/bypasswaf/场景合并
        ...
        if bool(t):
            for vul_flag in t:
                ...
                if 'GET' in data['method']:
                    ...
                    print(
                        'exist SQL Vulnerability___(Text comparison)  ' + str(datalist[vul_flag]) + '           ' + str(data_body[vul_flag]))
                    self.output_data(
                        'exist SQL Vulnerability___(Text comparison)  ' + str(datalist[vul_flag]) + '           ' + '\n'+str(data['headers']) + '\n'+str(data_body[vul_flag]))
                    self.output_html(htm_list[vul_flag]+'\n\n\n')
                    # print(htm_list[vul_flag])
                    # self.output_data(
                    #     'exist SQL Vulnerability___(Text comparison)  '+htm_list[vul_flag]+'\n\n\n\n\n'+htm_list[vul_flag+1])
        ...
        if waf_flag == 1:
            ...
            for ms in range(1,3):
                if ms == 1:
                    ...
                    self.test_json = self.get_json()
                    continue
                elif ms == 2 and 'json' in self.test_json['headers']['Content-Type']==False:
                    self.test_json['headers']['method'] = 'POST'
    ...
    def output_html(self, i):
        with open('vul_html.txt', "a", encoding='utf-8') as f:
            f.write(i + "\n")
